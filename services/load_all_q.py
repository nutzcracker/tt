...
from db.models import Query, MetricsQuery
...
async def add_data(data):
    async with async_session() as session:
        for query in data['text_indicator_to_statistics']:
            ...
            stmt = select(Query).where(Query.query == query_name)
            ...
            for el in query['statistics']:
                if date != el['date']:
                    stmt = select(MetricsQuery).where(
                        MetricsQuery.query == query_name,
                        MetricsQuery.date == datetime.strptime(date, date_format)
                    )
                    result = await session.execute(stmt)
